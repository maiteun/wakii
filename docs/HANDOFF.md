...
- ⚠️ **아직 실행 안 한 SQL (베타 전 필수)**:
  - **`supabase/groups.sql`** — 그룹(참여코드) 테이블. 돌려야 내가 만든 방의 코드가 기기 간 공유됨. (폴백 있어 안 돌려도 온보딩은 로컬 목업으로 진행.)
  - **`supabase/profiles.sql`** — 프로필 테이블. **이제 이메일이 PK**(email→name+avatar_url). ⚠️ 재실행 시 기존(이름 PK) 테이블 **drop 후 재생성**(베타라 OK). 돌려야 프로필 사진·이름이 팀원에게도 보임.
  - **`supabase/reactions_photo.sql`** — reactions에 `image_url` 컬럼 추가. 돌려야 즉석 원형 사진 반응이 DB에 영속·재생됨.
  - **`supabase/groups_avatar.sql`** — groups에 `avatar_url` 컬럼 추가. 돌려야 방 대표 사진이 기기 간 공유됨.
...
## 구현된 기능
- **베타 테스트 방법(2026-06-30)**: ① Supabase에서 `groups.sql`+`profiles.sql` 실행 → ② 배포본에서 그룹 만들 때 **참여 코드를 직접 입력**(=비번, 4자 이상, 대소문자 무시·대문자 저장) → ③ 그 코드를 팀원에게 공유 → ④ 팀원은 본인 이름 입력 후 같은 코드로 참여 → 사진/반응이 **실제 Supabase에 저장·실시간 공유**됨. ⚠️ 아직 진짜 로그인(OAuth) 없음 → "이름 = 신원". 같은 이름 쓰면 "내 글" 구분 꼬임. RLS 전부 열려 있어 **닫힌 가족 베타 한정**(외부 공개 전 잠가야 함).
- **신원 = 이메일(2026-06-30)**: 온보딩에서 **이메일+이름**을 받아 프로필 생성. **작성자 키(`author`/`card.who`)=이메일**(고유), 표시는 `nameOf(email)`로 이름 해석(`profileMap: email→{name,avatar}`). `mine`도 이메일 기준이라 동명이인 충돌 없음. `wakii.email`/`wakii.name` localStorage. ⚠️ 여전히 진짜 인증 아님(아무 이메일 입력 가능, OAuth 전까진 사칭 가능) — 닫힌 베타 한정.
- **프로필 사진(팀원 공유)**: 마이에서 사진 선택 → 다운스케일(`downscaleImage`) → Storage 업로드 → `profiles`(email) upsert → 팀원도 봄. `nameOf`/`avatarOf`로 표시. 현재 **완주 리캡 아바타**에 적용(방 카드·워키 스토리 아직 미적용 — walkRooms 하드코딩).
- **방 대표 사진**: 그룹 만들 때 사진 임베드(`groups.avatar_url`) → 홈 방 목록에서 🏠 대신 그 사진. 목업은 data URL.
- **온보딩**: 로그인 화면(소셜 버튼 UI만) → **이메일+이름**(프로필) → **우리 집 고르기** → 그룹 만들기(방 사진+이름+코드 직접 입력) / 코드로 참여 → 홈.
- **홈 정리(2026-06-30)**: "우리 집 길게 눌러 바꾸기" 멘트 제거(길게 탭 기능은 유지), 방 목록의 "가족 초대하기" 제거.
- **방 참여코드**: 방 화면 맨 아래 흐린 작은 글씨로 `참여코드 XXXX`(탭하면 복사). MVP라 눈에 잘 안 띄는 곳에 둠.
...
- **사진 내리기**: 덱 갤러리 **사진 우상단 빨간 ✕**(내 글 cards[0].mine일 때만 노출, 헤더의 닫기 ✕와 별개) → 확인 후 `deleteDeck`(decks 삭제 cascade로 cards/reactions + Storage 사진 제거). 목업은 로컬 상태에서 제거. ⚠️ UI 게이트만 있고 RLS는 열려 있어 API론 누구나 삭제 가능(보안 강화는 백로그).
- **즉석 사진 반응 영속(2026-06-30)**: 이모지 길게→즉석 원형 사진 반응이 이제 **덱에 저장돼 재진입 시 이모지 반응과 같은 촤르륵 모션으로 재생**됨. `Card.photoReactions:{emoji,img}[]`, `reactions.image_url` 컬럼, `recordPhotoReaction`. 재생 effect가 emoji+photo 한 풀로 섞어 spawn.
- **워키(걸음) — 코스 시스템 (A 구조)**: 코스 1개 = 랜드마크 1개(실측 15~20km대), 한 번에 1개만 진행, 가족 걸음 합산 공동 진행, 완주(100%) 시 0으로 리셋, 모든 코스 영구 재선택 가능.
  ...
  - **완주 리캡(B안 구현)**: 그룹 방 사진을 **작성자별로 묶어 가장 반응 많은 best 1장씩** 2열 그리드로(`curateRecap`, `.rc-grid`). 동점=이모지 종류 수, 반응 0이어도 사진 있으면 포함. 우상단 반응 수 배지, 하단 아바타+이름(`profileMap` 사진). 사진 없으면 안내 문구 폴백. ⚠️ **여정 기간 미반영**(코스별 startedAt 없음) → 지금은 방 전체 사진 기준. A안(완주 시점 스냅샷·기간별)으로 가려면 `curateRecap`의 deck 소스만 교체.
...
## 다음에 할 만한 것 (백로그)
1. **`supabase/groups.sql` + `profiles.sql` 실행** → 그룹 코드·프로필 사진 기기 간 실제 공유 (베타 전 필수).
2. **실제 소셜 OAuth**(카카오 우선) — Supabase Auth provider 설정 필요. (붙으면 "이름=신원" 한계 해소·이름 단계 생략.)
3. RLS 보안 강화(현재 프로토타입용으로 열려 있음) — 외부 공개 전 필수.
4. 걸음 수 실제 연동(헬스킷/구글핏) → `familyKm` 갱신에 연결 — 현재 목업(진행률이 시드/리셋으로만 변함).
5. 프로필 아바타 **방 카드·워키 스토리에도 적용**(현재 리캡만). 워키 스토리는 하드코딩 `walkRooms` → 실제 그룹 멤버로 교체 필요.
6. 완주 리캡 **A안**(완주 시점 스냅샷·여정 기간별 큐레이션) — 코스 active 시 `startedAt` 저장 + 완주 기록에 best 목록 고정.
7. 개발용 Supabase 분리(로컬 테스트가 실서버 DB에 쌓이는 문제).
8. 하단 글래스 네비 — Figma 프레임 기준(글래스 알약 3개 + 우측 원형 버튼 + 커스텀 아이콘)으로 아직 안 맞춤(전역 컴포넌트라 보류 중).
