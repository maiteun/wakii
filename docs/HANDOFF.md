...
- ⚠️ **아직 실행 안 한 SQL (베타 전 필수)**:
  ...
  - **`supabase/profiles.sql`** — 프로필(이름→아바타 사진) 테이블. 돌려야 내 프로필 사진이 팀원에게도 보임. (안 돌리면 아바타는 내 기기에서만 보임.)
  - **`supabase/reactions_photo.sql`** — reactions에 `image_url` 컬럼 추가. 돌려야 즉석 원형 사진 반응이 DB에 영속·재생됨. (안 돌리면 사진 반응은 그 세션에서만, 목업은 localStorage만.)
...
- **프로필 사진(팀원 공유)**: 마이에서 사진 선택 → Storage 업로드 → `profiles` 테이블(이름→avatar_url) 저장 → 팀원도 봄. `profileMap` state로 들고 있고 현재 **완주 리캡 아바타**에 적용(방 카드·워키 스토리는 아직 미적용 — walkRooms는 하드코딩 데모). 목업 모드는 localStorage만.
- **온보딩**: 로그인 화면(카카오/네이버/구글/Apple 버튼 — UI만, OAuth 미연동) → 이름 입력 → **우리 집 고르기** → 그룹 만들기(이름+코드 직접 입력) / 코드로 참여 → 홈.
